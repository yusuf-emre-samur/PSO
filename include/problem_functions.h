...
class ProblemFunctions {
  public:
    static double const drop_wave(pso::Position const& pos) {
        ...
        return -(1 +
                 std::cos(12 * std::sqrt(std::pow(x1, 2) + std::pow(x2, 2)))) /
    ...
    static double const ackley_function(pso::Position const& pos) {
        ...
        return -a *
            std::exp(-b *
                     std::sqrt(1 / 2 * (std::pow(x1, 2) + std::pow(x2, 2)))) -
            std::exp(1 / 2 * (std::cos(c * x1) + std::cos(c * x2))) + a +
            std::exp(1);
    }
    ...
    static double const levy_n13_function(pso::Position const& pos) {
        const double x1 = pos.x();
        const double x2 = pos.y();
        return std::pow(std::sin(3 * M_PI * x1), 2) +
            std::pow((x1 - 1), 2) * (1 + std::pow((3 * M_PI * x2), 2)) +
            std::pow((x2 - 1), 2) * (1 + std::pow((2 * M_PI * x2), 2));
    }
};
